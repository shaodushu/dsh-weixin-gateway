...
另外：
- **回复模式**（0.5.1+）：默认 `aggregate` 聚合模式——生成期间只显示"正在输入"，完成后统一发送：≤1500 字符一条成文；超长回复按段落切（每段 ≤500 字、Unicode 安全），段间 500ms 间隔防限流。避免 80 字符碎片把长回复切成多条小消息、以及同 context 高频连发被服务端拒绝（ret=-2）导致的"回复被截断"。可用环境变量 `DSH_WEIXIN_REPLY_MODE=stream` 切换回旧的流式增量模式（80 字符阈值实时发送）。
- 聚合模式超时 25s 未完成会先发一条"还在生成，请稍候～"占位提示（最多一次；文生图等已有专用占位则不重复）。
- **发送失败分流**（0.5.1+）：`ret=-2 rate limited`（真限流）→ 指数退避自动重试（1s/2s/4s，最多 3 次），恢复后内容自动补发；`ret=-2 prepare failed` 或裸 `-2`（context 过期/会话级失效）→ 首次即提示"⚠️ 消息发送通道异常（会话已失效）：请先给机器人发一条消息刷新会话"，且不再盲目重试（后续分片跳过）；其他错误连续失败 ≥3 次 → 现有兜底提示。不会静默丢回复（0.4.0+）。
- **单词截断保护**（0.4.1+，stream 模式）：流式回复按 80 字符阈值分段发送，当文本末尾以不完整拉丁单词（如 "km" 等待 "/h" 到达）结尾时，会把单词尾巴留在缓存中等待后续内容，防止"km/h"被劈成两段发到微信（实测模型在 "km" 与 "/h" 间停顿 670ms 触发阈值）。中文文本不受影响。

...
function parseField(raw, field) {
    ...
    if (field === 'dayOfWeek' && value === 7)
        value = 0; // 0 与 7 都是周日
    if (value < min || value > max) {
        throw new CronParseError(`cron 字段 ${field} 值 ${value} 超出范围 ${min}-${max}`, field);
    }
...
export function parseCronExpr(expr) {
    ...
    if (parts.length !== 5) {
        throw new CronParseError(`cron 表达式必须恰好 5 段（分 时 日 月 周），实际 ${parts.length} 段: "${expr}"`, 'minute');
    }
...
function matchField(spec, v) {
    return spec.any || spec.values.has(v);
}
/** 本地时间是否命中表达式（日字段用标准 OR 语义）。 */
export function matchesCronExpr(expr, d) {
    if (!matchField(expr.minute, d.getMinutes()))
        return false;
    if (!matchField(expr.hour, d.getHours()))
        return false;
    if (!matchField(expr.month, d.getMonth() + 1))
        return false;

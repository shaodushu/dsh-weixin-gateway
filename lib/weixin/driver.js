...
import { CDN_BASE_URL, DEFAULT_BASE_URL, clearStaleAccountsForUserId, loadWeixinAccount, saveWeixinAccount, triggerWeixinChannelReload, } from './accounts.js';
...
import { weixinMessageToMsgContext, restoreContextTokens, setContextToken, } from './inbound.js';
...
import { handleSlashMessage, isSlashCommandAuthorized, parseSlashCommand, resolveAdminUserIds, } from './slash-command.js';
...
async function handleIncoming(ctx, agentHandle, account, msg, baseUrl, token, configManager, router, sessionMode) {
    const to = msg.from_user_id ?? '';
    const contextToken = msg.context_token;
    // 持久化 context token：主动推送（cron / CLI push）的数据链地基——
    // 不入盘则 context-tokens.json 永远为空，独立进程拿不到发送凭证
    if (contextToken)
        setContextToken(account.accountId, to, contextToken);
    ...
    if (!mainMediaItem && text.startsWith('/')) {
        ...
        msgCtx.CommandAuthorized = isSlashCommandAuthorized(parseSlashCommand(text) ?? { name: '', args: [] }, { userId: to, adminUserIds, sessionMode });
        ...
        if (consumed)
            return; // 已被命令消费（含"命令执行失败"回复），不再走 agent
    }
    ...
    try {
        ...
        const replyMode = (process.env.DSH_WEIXIN_REPLY_MODE ?? 'aggregate');
        ...
        progressTimer = setTimeout(() => {
            if (replyMode !== 'aggregate' || progressSent)
                return;
            progressSent = true;
            void sendMessageWeixin({
                to,

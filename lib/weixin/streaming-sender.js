...
export class WeixinStreamingSender {
    ...
    async flush() {
        ...
        if (this.mode === 'aggregate') {
            ...
            logger.info(`weixin-gateway: aggregate reply sent (${this.sentCount} messages, ${textReply.length} chars)`);
            logger.debug(`weixin-gateway: streaming sender flushed (${this.sentCount} messages, ${this.mediaParts.length} media)`);
     ...
     */
    queueSend(text, withDelay = false) {
        const t = text.trim();
        if (!t)
            return;
    ...
    async sendWithRetry(text) {
        ...
        for (let attempt = 0; attempt < RETRY_MAX_ATTEMPTS; attempt++) {
            try {
                ...
                lastErr = err;
                if (!(err instanceof SendMessageError) || !err.retryable)
                    throw err;
 ...
 * - 段间不补分隔符，每段 trim
 */
export function splitAggregate(text) {
    const textReply = text.trim();
    if (!textReply)
        return [];
    if (Array.from(textReply).length <= AGGREGATE_SINGLE_MAX)
        return [textReply];

...
export function currentNickname(env = process.env) {
 ...
 * 空昵称视为不修改，原样返回。
 */
export function applyNicknameText(existing, nickname) {
    ...
    while (next.length > 0 && next[next.length - 1] === '')
        next.pop();
    if (!firstIndex.has(NICKNAME_ENV_KEY))
        next.push(`${NICKNAME_ENV_KEY}=${value}`);
    let text = next.join('\n');
    if (text !== '')
        text += '\n';
    return text;
...
export function writeNicknameEnv(text) {
    return writeEnvFile(text);
}
/** .env 文件路径（复用 ai-config 的固定位置）。 */
export function nicknameEnvPath() {
    return envPath();
}

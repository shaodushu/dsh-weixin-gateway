...
function topLevelRange(text, key) {
    const lines = text.split('\n');
 ...
 */
export function resolveDialogModelConfig(settingsText) {
...
/** 删除顶层块（连同紧邻空行清理）。 */
function deleteTopLevelBlock(text, key) {
    const range = topLevelRange(text, key);
    if (!range)
        return text;
    const lines = text.split('\n');
...
export function upsertCredentialKey(text, key, value) {
    if (text.trim() === '')
        return `{ ${key}: ${value} }\n`;
 ...
 */
export function applyDialogModelAnswers(settingsText, credText, answers) {
    ...
    if (!url || !key)
        return { settingsNext: settingsText, credNext: credText }; // 防御：交互层已保证必填
...
export function writeSettingsFile(text) {
    return writeTextFile(settingsPath(), text);
}
/** 写 .credentials.yaml（0600）。 */
export function writeCredentialsFile(text) {
    return writeTextFile(credentialsPath(), text);
}
function writeTextFile(p, text) {
    try {
...
export function summarizeDialogModel(settingsText) {
    const cfg = resolveDialogModelConfig(settingsText);
    if (!cfg)
        return '  ⚪ 对话模型: 未配置（可 dsh-weixin setup 引导，或手动编辑 ~/.dsh/settings.yaml）';

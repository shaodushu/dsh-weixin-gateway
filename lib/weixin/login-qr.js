    ...
    while (Date.now() < deadline) {
        try {
            ...
            switch (statusResponse.status) {
                ...
                case "confirmed": {
                    // 调试：打印完整 confirmed 响应（脱敏 token），排查服务端是否返回额外字段
                    const sanitized = JSON.stringify(statusResponse).replace(/(bot_token"\s*:\s*")[^"]+/, '$1***');
                    logger.info(`pollQRStatus: confirmed raw=${sanitized.substring(0, 800)}`);
                    ...
                    activeLogins.delete(opts.sessionKey);
                    logger.info(`✅ Login confirmed! ilink_bot_id=${statusResponse.ilink_bot_id} ilink_user_id=${redactToken(statusResponse.ilink_user_id)} baseurl=${statusResponse.baseurl} bot_token_prefix=${String(statusResponse.bot_token ?? '').slice(0, 12)}`);
                    return {
                        connected: true,

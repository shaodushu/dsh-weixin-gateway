...
function resolveTimeoutSec(envName, fallback) {
...
export async function askAgentStreaming(handle, text, cbs = {}) {
    ...
    const turnTimeoutSec = cbs.turnTimeoutSec ?? resolveTimeoutSec('OPENCLAW_ASK_TIMEOUT_SEC', DEFAULT_TURN_TIMEOUT_SEC);
    const idleTimeoutSec = cbs.idleTimeoutSec ?? resolveTimeoutSec('OPENCLAW_ASK_IDLE_TIMEOUT_SEC', DEFAULT_IDLE_TIMEOUT_SEC);
    let guard;
    try {
        agent.followup(createUserMessage({
            content: [{ type: 'text', text }],
        ...
        guard = withIdleTimeout(agent.whenIdle(), idleTimeoutSec * 1000, () => new Error(`AI 响应超时（${idleTimeoutSec}s 无输出），请稍后再试`));
        resetIdle = guard.reset;
        await raceWithTimeout(guard.promise, turnTimeoutSec * 1000, () => new Error(`任务处理超时（>${turnTimeoutSec}s），请稍后再试`));
        return summarize(agent.session.events, firstSeq);
    }
    finally {
        guard?.dispose();

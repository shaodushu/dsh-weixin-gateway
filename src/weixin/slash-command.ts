...
import type { SessionMode, SessionRouter } from './session-router.js'

export type SlashCommandName = 'help' | 'reset' | 'status' | 'cron'

export interface SlashCommandSpec {
  name: SlashCommandName
  description: string
  usage: string
  /** 需要管理员权限（WECHAT_ADMIN_IDS 白名单）。 */
 ...
 * status/cron 需要管理员。
 */
export function isSlashCommandAuthorized(
  cmd: { name: string },
  input: { userId: string; adminUserIds: ReadonlySet<string>; sessionMode: SessionMode },
): boolean {
  if (input.adminUserIds.has(input.userId)) return true
  const spec = SLASH_COMMANDS.find((s) => s.name === cmd.name)
  if (!spec) return false
  // reset 特殊：per-user 模式重置自己会话免授权；room 模式（共享会话）需管理员
  if (cmd.name === 'reset') return input.sessionMode === 'per-user'
  return !spec.requireAdmin
}

export type SlashRouteResult =
  | { kind: 'handled'; command: SlashCommandName; args: string[] }
  | { kind: 'not-command' }
  /** 以 / 开头但未命中命令表 → 放行给 agent。 */
  | { kind: 'unknown' }
  | { kind: 'not-authorized' }
...
export function routeSlashCommand(input: {
  ...
  if (!isSlashCommandAuthorized(parsed, input)) return { kind: 'not-authorized' }
  return { kind: 'handled', command: spec.name, args: parsed.args }
}

/** 管理员列表解析：WECHAT_ADMIN_IDS 逗号分隔、trim、去空（纯函数，env 注入）。 */
export function parseAdminUserIds(env: NodeJS.ProcessEnv): ReadonlySet<string> {
 ...
 */
export function resolveAdminUserIds(): ReadonlySet<string> {
  loadEnvFile()
...
export interface SlashExecutorDeps {
  ...
  /** driver 注入：sendMessageWeixin 包装（带 contextToken）。 */
  send: (text: string) => Promise<void>
  /** 测试注入；缺省 loadCronJobs。 */
  loadJobs?: () => CronJob[]
}
 ...
 */
export async function handleSlashMessage(
  ...
  if (route.kind === 'not-command' || route.kind === 'unknown') return false
  try {
...
function buildStatusText(deps: SlashExecutorDeps): string {
  return [
    '⚙️ 网关状态',
    `账号: ${deps.accountId}`,

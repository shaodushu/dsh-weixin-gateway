...
export function lockPath(accountId: string): string {
  return path.join(resolveStateDir(), 'weixin-dsh', `run-${accountId}.lock`)
}

/** 读锁文件内记录的第一行（PID）；文件缺失/内容为空返回 undefined。 */
function readLockPid(lockFile: string): string | undefined {
  try {
...
function isGatewayProcess(pid: string): boolean {
...
function writeLock(accountId: string): void {
 ...
 */
export function acquireRunLock(accountId: string): { ok: true } | { ok: false; pid: string } {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (pid && isGatewayProcess(pid)) return { ok: false, pid }
      // 残留锁 → 删掉重试
...
export function releaseRunLock(accountId: string): void {
  fs.rmSync(lockPath(accountId), { force: true })
}

/** 扫描所有被存活网关实例持有的锁（cli 转发前预检用，只读不写）。 */
export function findHeldGatewayLocks(): Array<{ accountId: string; pid: string }> {
  ...
  const held: Array<{ accountId: string; pid: string }> = []
  for (const f of files) {
    if (!f.startsWith('run-') || !f.endsWith('.lock')) continue

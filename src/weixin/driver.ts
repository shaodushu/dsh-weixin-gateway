...
async function handleIncoming(
  ...
  if (mediaOpts.decryptedVoicePath) {
    ...
    logger.info(
      `weixin-gateway: inbound voice_item=${JSON.stringify({
  ...
  try {
    ...
    const result = await askAgentStreaming(agentHandle, prompt, {
      onDelta: (delta) => void sender.feed(delta),
    })
 ...
 */
class WeixinStreamingSender {
  /** 待发送的普通文本累积（含不完整行）。 */
  private pending = ''
  /** markdown 安全分片过滤器。 */
  private readonly filter = new StreamingMarkdownFilter()
  /** 收集的媒体标记。 */
  private readonly mediaParts: Array<{ path: string; caption?: string }> = []
  /** 收集的 [tts:] 文本。 */
  ...
  /** 接收一个文本增量（assistant/chunk text-delta）。 */
  feed(delta: string): void {
    ...
    const extracted = this.extractMarkers()
    if (this.pending.length >= this.threshold) {
      this.queueSend(this.pending)
      this.pending = ''
    }
    void extracted
  }

...
async function handleIncoming(
  ...
  const mediaOpts = {} as import('./inbound.js').WeixinInboundMediaOpts
  if (mainMediaItem) {
    try {
  ...
  try {
    ...
    const result = await askAgent(agentHandle, prompt)
    ...
    // 解析媒体标记 [image:路径] / [video:路径] / [file:路径]：先发媒体，再发剩余文本
    const { mediaParts, textParts } = splitMediaMarkers(reply)
...
function pickMediaItem(full: WeixinMessage): MessageItem | undefined {
...
/**
 * 解析回复文本中的媒体标记：
 *   [image:/path/a.png] [video:/path/b.mp4] [file:/path/c.pdf]
 * 或 markdown 图片语法 ![说明](/path/a.png)
 * 返回媒体部分与剩余文本部分。
 */
function splitMediaMarkers(reply: string): {
  mediaParts: Array<{ path: string; caption?: string }>
  textParts: string[]
} {
  const mediaParts: Array<{ path: string; caption?: string }> = []
  const textParts: string[] = []
  const lines = reply.split('\n')
  for (const line of lines) {
    const bracket = line.match(/^\[(image|video|file):\s*([^\]]+)\]\s*(.*)$/)
    const md = line.match(/^!\[([^\]]*)\]\(([^)]+)\)\s*(.*)$/)
    if (bracket) {
      mediaParts.push({ path: bracket[2].trim(), caption: bracket[3] })
    } else if (md) {
      mediaParts.push({ path: md[2].trim(), caption: md[1] })
    } else {
      textParts.push(line)
    }
  }

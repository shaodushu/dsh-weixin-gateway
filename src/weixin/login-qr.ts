    ...
    try {
      ...
      switch (statusResponse.status) {
        ...
        case "confirmed": {
          ...
          logger.info(
            `✅ Login confirmed! ilink_bot_id=${statusResponse.ilink_bot_id} ilink_user_id=${redactToken(statusResponse.ilink_user_id)} baseurl=${statusResponse.baseurl} bot_token_prefix=${String(statusResponse.bot_token ?? '').slice(0, 12)}`,
          );

...
function listLocalAccounts(): string[] {
  try {
    const raw = fs.readFileSync(
      path.join(resolveStateDir(), 'weixin-dsh', 'accounts-index.json'),
      'utf8',
    )
 ...
 */
export function resolvePushAccount(accountId?: string): ResolvedPushAccount {
 ...
 */
export function resolvePushTargets(accountId: string, to: string): PushTarget[] {
  if (to === 'all') {
...
export async function pushMessage(params: {
  ...
  to: string
  text: string
  /** 测试注入；缺省走 sendMessageWeixin。 */
  sendFn?: (to: string, text: string, contextToken: string | undefined) => Promise<void>
}): Promise<PushResult> {
  const acct = resolvePushAccount(params.accountId)
  const targets = resolvePushTargets(acct.accountId, params.to)
  if (targets.length === 0) {
    throw new Error(
      `没有可推送的目标：账号 ${acct.accountId} 下没有活跃会话用户（用户需先给机器人发过消息）`,
    )
  }
  const sendFn =
    params.sendFn ??

...
import type { CronExpr } from './cron-expr.js'
...
export type CronJobType = 'text' | 'prompt'

export interface CronJob {
  /** generateId('cron')：`cron:{ts}-{hex}`。 */
  id: string
  /** 5 字段 cron 表达式原文。 */
  cron: string
  /** text：静态发送内容；prompt：发给 agent 的提示词（到时生成后发送）。 */
  content: string
  type: CronJobType
  /** 具体 userId 或 'all'（账号下所有活跃会话用户）。 */
  to: string
  /** 归属账号；缺省 = 执行时当前运行的网关账号。 */
  accountId?: string
  createdAt: string
  /** 上次执行（或错过推进）时刻，防重启重复发送。 */
  lastSentAt?: string
}
  ...
  try {
    ...
    return parsed.filter(
      (j): j is CronJob =>
        ...
        typeof (j as CronJob).cron === 'string' &&
        typeof (j as CronJob).content === 'string',
    )
  } catch {
    return []
  }
...
export function updateCronJobLastSentAt(id: string, at: string): void {
  const jobs = loadCronJobs()
...
export function nextRunAtOf(job: CronJob, from: Date = new Date()): Date | null {
  let expr: CronExpr
  try {
...
/** 单个任务的到期状态。 */
export type JobDueState =
  | { kind: 'due'; scheduledFor: Date }
  /** 错过宽限窗口：不发送，但需把 lastSentAt 推进到该触发时刻防卡死。 */
  | { kind: 'missed'; scheduledFor: Date }
  | { kind: 'not-due' }
 ...
 */
export function evalJobDue(
  job: CronJob,
  now: Date,
  accountId: string,
): JobDueState {
  if (job.accountId && job.accountId !== accountId) return { kind: 'not-due' }
  const ref = job.lastSentAt ? new Date(job.lastSentAt) : now
  const scheduled = nextRunAtOf(job, ref)
  if (!scheduled) return { kind: 'not-due' }
  if (now.getTime() < scheduled.getTime()) return { kind: 'not-due' }
  const overdue = now.getTime() - scheduled.getTime()
  if (overdue <= CRON_GRACE_MS) return { kind: 'due', scheduledFor: scheduled }
...
export function findDueJobs(
  ...
  now: Date,
  accountId: string,
): { job: CronJob; scheduledFor: Date }[] {
  const due: { job: CronJob; scheduledFor: Date }[] = []
  for (const job of jobs) {
    const state = evalJobDue(job, now, accountId)
    if (state.kind === 'due') due.push({ job, scheduledFor: state.scheduledFor })
  }
  return due
}
 ...
 */
export function formatCronJobList(jobs: CronJob[], now: Date = new Date()): string {
  if (jobs.length === 0) return '📋 定时任务：暂无'

 ...
 */
export function decompressAllFrames(raw: Buffer): Buffer {
...
/**
 * 逐行扫描 zstd 会话文件并验证 seq 连续性；损坏时重建文件保留有效前缀。
 *
 * - 扫描成功（全部事件 seq 连续）→ 'ok'，文件不动
 * - 扫描遇坏行 → 重建文件（header 帧 + 有效前缀帧，zstd checksum），返回
 *   'repaired'；调用方随后应从修复后的文件重建会话
 * - 解压失败（帧级损坏，无法恢复任何前缀）→ 'unrepairable'，文件不动，
 *   调用方保持内存会话并跳过本次同步
 */
/** 扫描结果。 */
export type SessionScanOutcome = 'ok' | 'repaired' | 'unrepairable'

export interface SessionScan {
  outcome: SessionScanOutcome
  /** 有效事件数（含本次扫描验证通过的全部事件）。 */
  validEvents: number
}

export async function scanAndRepairSessionLog(file: string): Promise<SessionScan> {
    let raw;
    try {

...
/** 能力定义：环境变量组前缀 + 默认模型 + 网关端点路径。 */
export interface AiCapabilityDef {
  id: AiCapabilityId
  /** 展示名（引导与摘要用）。 */
  label: string
  /** 环境变量组前缀（AI_ASR_ / AI_VISION_ / AI_IMAGE_ / AI_TTS_）。 */
  prefix: string
  /** 未显式配置 MODEL 时使用的模型。 */
  defaultModel: string
  /** 网关端点路径（追加在 baseUrl 后）。 */
  endpointPath: string
}
...
/** 按 id 取能力定义。 */
export function capabilityDef(id: AiCapabilityId): AiCapabilityDef {
  const def = AI_CAPABILITIES.find((d) => d.id === id)
  if (!def) throw new Error(`未知 AI 能力: ${id}`)
  return def
}
...
export function resolveCapabilityConfig(id: AiCapabilityId, env: NodeJS.ProcessEnv): AiCapabilityConfig | null {
...
export function getCapabilityConfig(id: AiCapabilityId): AiCapabilityConfig | null {
  loadEnvFile()
  return resolveCapabilityConfig(id, process.env)
}

/** 同步判定该能力是否已配置（网关注册工具用）。 */
export function isCapabilityConfigured(id: AiCapabilityId): boolean {
  return getCapabilityConfig(id) !== null
}

/** 未配置抛错，消息含能力名与所需变量名（ai-service 四个能力函数共用）。 */
export function requireCapabilityConfig(id: AiCapabilityId): AiCapabilityConfig {
  const cfg = getCapabilityConfig(id)
  if (!cfg) {
    const def = capabilityDef(id)
    throw new Error(
      `缺少 ${def.prefix}BASE_URL/${def.prefix}KEY（或全局 AI_GATEWAY_BASE_URL/AI_GATEWAY_KEY）：${def.label}未配置`,
    )
  }
  return cfg
}
...
// ---------------------------------------------------------------------------
// setup 交互式引导（纯逻辑，无交互副作用，便于单测）
// ---------------------------------------------------------------------------

/** 单个问题。required 用于密钥必填循环。 */
export interface AiConfigQuestion {
  /** `${capId}.url` / `.key` / `.model` / `.action`。 */
  id: string
  /** 已含当前值/默认值提示（密钥打码显示）。 */
  prompt: string
  defaultValue?: string
...
/** 单能力的问答束。 */
export interface AiCapabilityQuestions {
  capId: AiCapabilityId
  label: string
  configured: boolean
  /** 解析现值（含全局 fallback 与默认模型），预填用。 */
  current: AiCapabilityConfig | null
  /** 已配置时的三选一（回车保持 / r 重配 / x 清除）。 */
  action?: AiConfigQuestion
  /** url / key / model 三问。 */
...
export function buildAiConfigQuestions(env: NodeJS.ProcessEnv = process.env): AiCapabilityQuestions[] {
 ...
 */
export function applyAiEnvAnswers(existing: string, answers: Record<string, string>): string {
  ...
  for (const [k, v] of Object.entries(answers)) {
    ...
    if (field === 'action' && v === 'x') {
      ...
      const envKey =
        field === 'url' ? `${def.prefix}BASE_URL` : field === 'key' ? `${def.prefix}KEY` : `${def.prefix}MODEL`
      toWrite.set(envKey, v)
    }
  }
  for (const k of toDelete) toWrite.delete(k)
  if (toDelete.size === 0 && toWrite.size === 0) return existing
  ...
  for (const [key, value] of toWrite) {
    if (firstIndex.has(key)) continue // 已原地替换
    next.push(`${key}=${value}`)
  }
  let text = next.join('\n')
  if (text !== '') text += '\n'
...
/** 非交互环境的手动配置指引文本。 */
export function aiEnvGuide(): string {
  const rows = AI_CAPABILITIES.map(
...
export function summarizeAiConfig(env: NodeJS.ProcessEnv = process.env): string {
  return AI_CAPABILITIES.map((def) => {
    const cfg = resolveCapabilityConfig(def.id, env)
    if (!cfg) return `  ⚪ ${def.label}: 未配置 → 静默降级`

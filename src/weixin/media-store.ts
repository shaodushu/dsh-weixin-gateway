...
const MAGIC_EXT: Array<{ bytes: number[]; ext: string }> = [
...
function inferExtension(buf: Buffer): string {
  for (const { bytes, ext } of MAGIC_EXT) {
    if (buf.length >= bytes.length && bytes.every((b, i) => buf[i] === b)) {
      // RIFF: 偏移 8 处的四字符码区分 WEBP / WAVE
      if (ext === '.webp' && buf.length >= 12 && buf.toString('ascii', 8, 12) !== 'WEBP') continue
      if (ext === '.wav' && buf.length >= 12 && buf.toString('ascii', 8, 12) !== 'WAVE') continue
      return ext
    }
  }
  return '.bin'

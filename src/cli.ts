...
async function updateCli(): Promise<void> {
  const view = spawnSync(
    'npm',
    ['view', 'dsh-weixin-gateway', 'version', '--registry', 'https://registry.npmjs.org', '--prefer-online'],
    { encoding: 'utf8' },
  )
  ...
  const install = spawnSync(
    'npm',
    ['install', '-g', `dsh-weixin-gateway@${latest}`, '--registry', 'https://registry.npmjs.org', '--prefer-online'],
    { stdio: 'inherit' },
  )
  if (install.status !== 0) {
 ...
 * 包根目录 .env（保留无关行与旧 AI_GATEWAY_* 全局凭据）。
 * 非 TTY（CI/管道）只打印环境变量指引，不阻塞。
 */
async function configureAiInteractively(): Promise<void> {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.log('\n[dsh-weixin] 非交互终端，跳过 AI 能力配置问答。可用环境变量或包根目录 .env 配置：')
  ...
  try {
    for (const b of bundles) {
      if (b.configured && b.action) {
        ...
        if (a === 'r') await askAiFields(rl, b, answers)
      } else {
...
async function askAiFields(
  rl: ReturnType<typeof createInterface>,
  b: AiCapabilityQuestions,
  answers: Record<string, string>,
): Promise<void> {
  for (const q of b.fields) {
    let input = (await rl.question(q.prompt)).trim()
    if (q.required) {
      while (input === '') input = (await rl.question(q.prompt)).trim()
    }
